...
# import random
#
# class HighDimensionalMapping:
#     """
#     这个类用于将图像映射到高维空间，进行变换后再投影回原始空间，以实现数据增强。
#     """
#
#     def __init__(self, noise_std=1, p=0.5):
#         """
#         初始化高维空间映射增强类。
#
#         Args:
#             noise_std (float): 在高维空间中添加的高斯噪声的标准差。默认值为2。
#             p (float): 应用数据增强的概率，范围为[0, 1]。默认值为0.5。
#         """
...
#
#         Args:
#             labels (dict): 包含'img'键的字典，其中存储要增强的图像。
#
#         Returns:
#             labels (dict): 包含增强后图像的字典，图像存储在'img'键中。
#         """
#         # 随机决定是否应用数据增强
#         if random.random() < self.p:
#             img = labels["img"]
#
#             # 确保图像是NumPy数组，形状为(H, W, C)
#             h, w, c = img.shape
#
#             # 将图像展开为二维数组 (像素数, 通道数)
...
#
#             # 定义高维映射函数
#             def high_dimensional_mapping(x):
#                 # x的形状为 (像素数, 通道数)
#                 # 对每个通道进行多项式和三角函数扩展
#                 features = [x]  # 原始特征
#                 features.append(x ** 2)
...
#
#             # 映射到高维空间
#             high_dim_features = high_dimensional_mapping(img_flat)
#
#             # 在高维空间中添加高斯噪声
#             noise = np.random.normal(0, self.noise_std, high_dim_features.shape)
#             high_dim_features_noisy = high_dim_features + noise
#
#             # 定义逆映射函数
#             def inverse_mapping(features):
#                 # 简化处理，取前c个特征作为重构后的像素值
#                 x_reconstructed = features[:, :c]
#                 # 将像素值归一化到[0, 255]
#                 x_min = x_reconstructed.min(axis=0, keepdims=True)
#                 x_max = x_reconstructed.max(axis=0, keepdims=True)
#                 x_reconstructed = (x_reconstructed - x_min) / (x_max - x_min + 1e-8)
#                 x_reconstructed = x_reconstructed * 255
#                 return x_reconstructed
#
#             # 投影回原始空间
#             img_projected_flat = inverse_mapping(high_dim_features_noisy)
#
#             # 确保像素值在有效范围内并转换为uint8类型
#             img_projected = np.clip(img_projected_flat, 0, 255).astype(np.uint8).reshape(h, w, c)
#
#             # 更新图像到labels字典中
#             labels["img"] = img_projected
#
#         # 如果不应用数据增强，直接返回原始labels
#         return labels
